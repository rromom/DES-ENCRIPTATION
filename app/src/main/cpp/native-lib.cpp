...
Java_com_example_myapplicationc_MainActivity_RESP1(JNIEnv *env, jobject object){
    return env->NewStringUTF(resp1.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_myapplicationc_MainActivity_GetTimeEnc(JNIEnv *env, jobject object){
return env->NewStringUTF(time_enc.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_myapplicationc_MainActivity_GetTimeDes(JNIEnv *env, jobject object){
    return env->NewStringUTF(time_des.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_myapplicationc_MainActivity_GetTime(JNIEnv *env, jobject object){
    return env->NewStringUTF(time_total.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_myapplicationc_MainActivity_GetMEM(JNIEnv *env, jobject object){
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_example_myapplicationc_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
    std::string hello = "Ingrese el Texto";
    return env->NewStringUTF(hello.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_myapplicationc_MainActivity_SetImage(JNIEnv *env, jobject object, jstring s, jstring k){
    std::string text=env->GetStringUTFChars(s, 0);
    std::string llav=env->GetStringUTFChars(k, 0);
    __android_log_print(ANDROID_LOG_DEBUG, "", "%s", text.c_str());
    __android_log_print(ANDROID_LOG_DEBUG, "", "%s", llav.c_str());
    return 0;

}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_myapplicationc_MainActivity_GetParams(JNIEnv *env, jobject object, jstring s, jstring k){

        unsigned t0_enc, t1_enc, t0_des, t1_des, tt1, tt2;
        tt1 = clock();
        //mem1= mem_total();
        std::string text = env->GetStringUTFChars(s, 0);
        std::string llav = env->GetStringUTFChars(k, 0);
        ...
        t0_enc = clock();
        DES_Act(encrypt_text, texto, sizeof(texto), llave, sizeof(llave), ENCRYPT);
        t1_enc = clock();
        ...
        //resp1 = TextToBinaryString(encrypt_text);

        //return env->NewStringUTF(TextToBinaryString(encrypt_text).c_str());
        t0_des = clock();
        DES_Act(decrypt_text, encrypt_text, sizeof(texto), llave, sizeof(texto), DECRYPT);
        t1_des = clock();
